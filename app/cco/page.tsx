...
export default function CCOPage() {
    ...
    const performSave = async () => {
      try {
        ...
        const fieldNames: Record<string, string> = { 
          ...
          tegRailwayMoega01: "TEG Ferr. 01 (Célula)",
          tegRailwayMoega01Operation: "TEG Ferr. 01 (Operação)",
          tegRailwayMoega02: "TEG Ferr. 02 (Célula)",
          tegRailwayMoega02Operation: "TEG Ferr. 02 (Operação)",
          teagRoad: "TEAG Rodovia",
          teagRailway: "TEAG Ferrovia",
          teagRoadTombador05: "TEAG Rod. 05", 
          teagRailwayMoega03: "TEAG Ferr. 03 (Célula)",
          teagRailwayMoega03Operation: "TEAG Ferr. 03 (Operação)",
          teagRailwayMoega04: "TEAG Ferr. 04 (Célula)",
          teagRailwayMoega04Operation: "TEAG Ferr. 04 (Operação)",
          teagRailwayMoega05: "TEAG Ferr. 05 (Célula)",
          teagRailwayMoega05Operation: "TEAG Ferr. 05 (Operação)"
        };

        const alertMessage = `🚨 ALTERAÇÃO: ${fieldNames[field]} alterado para ${value}`;
        ...
        toast({ title: "Campo Atualizado", description: `${fieldNames[field]} foi definido como ${value}.` });

      } catch (error) {
        console.error("Save failed:", error);
        toast({title: "Erro ao salvar alteração", variant: "destructive"});
        // Optional: Revert the state if save fails
        // listenToStorage(setStorageSelection); // Re-fetch from DB
      }
    };
  ...
  const formatChanges = (changes: StorageLog['changes']) => {
    return Object.entries(changes).filter(([, value]) => value).map(([key, value]) => {
      const prettyKey = key
          .replace('teg', 'TEG ').replace('teag', 'TEAG ').replace('Road', 'Rod.')
          .replace('Railway', 'Ferr.').replace('Moega', 'M.').replace('Tombador', 'Tomb.')
                      ...
                      <div className="space-y-4">
                          ...
                          <div className="space-y-1.5">
                              ...
                              <div className="flex gap-2">
                                ...
                                <Select id="teg-railway-moega-01-operation" onValueChange={(v) => handleStorageChange("tegRailwayMoega01Operation", v)} value={storageSelection?.tegRailwayMoega01Operation || ""}><SelectTrigger className="w-1/2"><SelectValue placeholder="Operação..." /></SelectTrigger><SelectContent><SelectItem value="Descarga Caminhão">Descarga Caminhão</SelectItem><SelectItem value="Descarga Vagão">Descarga Vagão</SelectItem></SelectContent></Select>
                              </div>
                          </div>
                          <div className="space-y-1.5">
                              ...
                              <div className="flex gap-2">
                                ...
                                <Select id="teg-railway-moega-02-operation" onValueChange={(v) => handleStorageChange("tegRailwayMoega02Operation", v)} value={storageSelection?.tegRailwayMoega02Operation || ""}><SelectTrigger className="w-1/2"><SelectValue placeholder="Operação..." /></SelectTrigger><SelectContent><SelectItem value="Descarga Caminhão">Descarga Caminhão</SelectItem><SelectItem value="Descarga Vagão">Descarga Vagão</SelectItem></SelectContent></Select>
                              </div>
                          </div>
                      </div>
                      <div className="space-y-4">
                          ...
                          <div className="space-y-1.5">
                            <label className="text-sm font-medium text-muted-foreground">Ferrovia - Moega 03:</label>
                             <div className="flex gap-2">
                                ...
                                <Select id="teag-railway-moega-03-operation" onValueChange={(v) => handleStorageChange("teagRailwayMoega03Operation", v)} value={storageSelection?.teagRailwayMoega03Operation || ""}><SelectTrigger className="w-1/2"><SelectValue placeholder="Operação..." /></SelectTrigger><SelectContent><SelectItem value="Descarga Caminhão">Descarga Caminhão</SelectItem><SelectItem value="Descarga Vagão">Descarga Vagão</SelectItem></SelectContent></Select>
                            </div>
                          </div>
                          <div className="space-y-1.5">
                            <label className="text-sm font-medium text-muted-foreground">Ferrovia - Moega 04:</label>
                             <div className="flex gap-2">
                                ...
                                <Select id="teag-railway-moega-04-operation" onValueChange={(v) => handleStorageChange("teagRailwayMoega04Operation", v)} value={storageSelection?.teagRailwayMoega04Operation || ""}><SelectTrigger className="w-1/2"><SelectValue placeholder="Operação..." /></SelectTrigger><SelectContent><SelectItem value="Descarga Caminhão">Descarga Caminhão</SelectItem><SelectItem value="Descarga Vagão">Descarga Vagão</SelectItem></SelectContent></Select>
                            </div>
                          </div>
                          <div className="space-y-1.5">
                            <label className="text-sm font-medium text-muted-foreground">Ferrovia - Moega 05:</label>
                             <div className="flex gap-2">
                                ...
                                <Select id="teag-railway-moega-05-operation" onValueChange={(v) => handleStorageChange("teagRailwayMoega05Operation", v)} value={storageSelection?.teagRailwayMoega05Operation || ""}><SelectTrigger className="w-1/2"><SelectValue placeholder="Operação..." /></SelectTrigger><SelectContent><SelectItem value="Descarga Caminhão">Descarga Caminhão</SelectItem><SelectItem value="Descarga Vagão">Descarga Vagão</SelectItem></SelectContent></Select>
                            </div>
                          </div>
                      </div>
                  </div>
